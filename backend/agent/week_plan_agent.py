...
def _suggest_weekly_meals(pantry_items: list[str], dietary: str, count: int = 5) -> tuple[list[dict[str, str]], str]:
    prompt = f"""You are SmartCart's meal planning assistant.
Return ONLY valid JSON.
Suggest {count} practical dinners for one week.
...
Output format:
{{"meals":[{{"name":"meal name","reason":"brief reason under 80 chars"}}]}}
"""
    try:
...
def build_week_plan(
    ...
    *,
    ...
    steps.append({
        ...
        "status": "success" if meal_source == "generated" else "error",
