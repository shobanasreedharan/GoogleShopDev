...
def get_stores_in_city(city: str, state: str, country: str = "US") -> list:
    ...
    if not stores:
        print(f"[store_prices] no city_prices stores for {city_key}; checking legacy store_prices")
        for store_id, store_data in _iter_legacy_store_price_docs(city, state, country):
            stores.append({
                "store_id": store_id,
                "store_name": store_data.get("store_name") or store_id,
                "city": normalize_city(store_data.get("city") or city),
                "state": normalize_state(store_data.get("state") or state),
                "country": normalize_country(store_data.get("country") or country),
                "currency": store_data.get("currency") or get_currency_for_country(store_data.get("country") or country),
